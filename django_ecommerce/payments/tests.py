...
import unittest
from .views import sign_in, sign_out, soon, register, Customer
...
class FormTesterMixin():
	...
	def assertFormError(self, form_cls, expected_error_name,
		...
		self.assertEquals(
			...
			expected_error_msg,
			msg="Expected {} : Actual {} : using_data {}".format(
				test_form.errors[expected_error_name],
...
class form_simpleTestCase(SimpleTestCase):
...
class CustomerTests(TestCase):
	...
	def test_create_subscription(self):
		with mock.patch('stripe.Customer.create') as create_mock:
			cust_data={'descrition': 'test user',
			'email': 'test@test.com', 'card': '4242', 'plan':'gold'}
